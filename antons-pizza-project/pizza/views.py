...
def order(request):
    ...
    if request.method == 'POST':
        ...
        if filled_form.is_valid():
            ...
            note = 'Thanks for ordering sucka! Your %s %s and %s pizza is on its way!' %(
                filled_form.cleaned_data['size'], 
                filled_form.cleaned_data['topping1'], 
                filled_form.cleaned_data['topping2'],)
            filled_form = PizzaForm()
        else:
            note = 'Pizza order has fialed. Try again'
...
def pizzas(request):
    ...
    if request.method == 'POST':
        ...
        if filled_formset.is_valid():
            ...
            note = 'Pizzas have been ordered sucka'
        else:
            note = 'Order failed, please retry'
...
def edit_order(request, pk):
    ...
    if request.method == 'POST':
        ...
        if filled_form.is_valid():
            filled_form.save()
            form = filled_form
            note = 'Order has been updated sucka.'
